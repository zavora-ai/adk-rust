...
use super::config::{MISTRAL_API_BASE, MistralConfig};
...
/// Mistral AI client backed by the shared OpenAI-compatible implementation.
///
/// # Example
///
/// ```rust,ignore
...
impl MistralClient {
    ...
    /// fails to initialize.
    pub fn new(config: MistralConfig) -> Result<Self, AdkError> {
        let MistralConfig { api_key, model, base_url } = config;
        let base_url = base_url.unwrap_or_else(|| MISTRAL_API_BASE.to_string());
