...
async fn main() -> Result<()> {
    ...
    // Get A2UI tools (render_screen, render_page, render_kit)
    let ui_tools = UiToolset::all_tools();

    // Create agent with A2UI prompt guidance
    ...
    println!("  - \"Create a welcome screen\"");
    println!("  - \"Show a support ticket form\"");
    println!("  - \"Build a dashboard page\"");
    println!();
    println!("The agent will use render_screen to generate A2UI JSONL messages.");
    println!();
