//! Anthropic Server-Side Tools — live integration example.
//!
//! Demonstrates Anthropic's web search tool (`web_search_20250305`) running
//! server-side alongside user-defined function calling tools. The web search
//! tool is executed by Anthropic's servers and returns `ServerToolCall` /
//! `ServerToolResponse` parts in the response stream.
//!
//! Scenarios:
//!   1. Web search tool + function tool coexistence (full agent round-trip)
//!   2. ServerToolCall/ServerToolResponse part inspection
...
/// Build a GenerateContentConfig with Anthropic web search tool in extensions.
...
///
/// The agent has Anthropic's web_search (server-side) and a custom get_capital
/// tool (function calling, client-side). When the user asks a question that
/// triggers web search, the agent should:
///   - Preserve ServerToolCall/ServerToolResponse parts from the web search
...
async fn test_web_search_plus_function_tool() -> anyhow::Result<()> {
    ...
    let agent = Arc::new(
        LlmAgentBuilder::new("anthropic-mixed-tools")
...
async fn test_function_tool_still_works() -> anyhow::Result<()> {
    ...
    let agent = Arc::new(
        LlmAgentBuilder::new("anthropic-function-tool")
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Anthropic Server-Side Tools — Live Integration Example");
    println!("======================================================");
    println!("Demonstrates web_search + function tools on Claude\n");
