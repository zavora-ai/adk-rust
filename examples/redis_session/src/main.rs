use adk_session::{
    CreateRequest, DeleteRequest, Event, EventActions, GetRequest, ListRequest, RedisSessionConfig,
...
/// Default Redis URL — matches the Docker container from the steering doc.
/// Start with: docker run -d --name adk-redis-test -p 6399:6379 redis:7-alpine
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== ADK Redis Session Example ===\n");

    // 1. Connect
    println!("1. Connecting to Redis...");
    let config = RedisSessionConfig { url: REDIS_URL.to_string(), ttl: None, cluster_nodes: None };
