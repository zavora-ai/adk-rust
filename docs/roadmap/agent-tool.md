...
Current multi-agent patterns in ADK-Rust have limitations:

- **Sub-agents**: Require explicit transfer, not callable as tools
- **Workflow Agents**: Limited to predefined execution patterns
- **Tool Composition**: Cannot compose agents with different tool types
- **Reusability**: Difficult to reuse agents across different contexts

## Planned Solution

AgentTool will wrap an agent as a tool, enabling:
...
```rust,ignore
use adk_agent::{LlmAgentBuilder, AgentTool};
...
// Wrap it as a tool
let math_tool = AgentTool::new(math_agent, None);
...
let data_analyst = LlmAgentBuilder::new("data_analyst")
    .model("gemini-2.0-flash-exp")
...
// Wrap as tool - automatically uses schemas
let analyst_tool = AgentTool::new(data_analyst, None);
```

### Configuration Options

```rust,ignore
use adk_agent::AgentToolConfig;
...
### Phase 1: Core AgentTool (Week 1)
- [ ] Create `AgentTool` struct wrapping an `Agent`
- [ ] Implement `Tool` trait for `AgentTool`
- [ ] Generate function declaration from agent metadata
- [ ] Handle input schema validation
- [ ] Handle output schema validation

### Phase 2: Execution Integration (Week 2)
- [ ] Create isolated session for sub-agent
- [ ] Forward parent state to sub-agent (filtered)
- [ ] Execute sub-agent and collect results
- [ ] Extract and validate output
- [ ] Handle errors and timeouts

### Phase 3: Advanced Features (Week 2-3)
- [ ] Implement artifact forwarding
- [ ] Add summarization control
- [ ] Support streaming responses
- [ ] Add telemetry and tracing
- [ ] Implement timeout handling

### Phase 4: Testing & Documentation (Week 3)
- [ ] Unit tests for AgentTool
- [ ] Integration tests with various agent types
- [ ] Example: Multi-domain coordinator
- [ ] Example: Specialized tool agents
