...
| builds `DeferredNodeConfig` with a struct literal | add `min_predecessors: None` |
| calls `get_next_nodes` or `time_travel` | handle the returned `Result` |
...
`DeferredNodeConfig` gained the public field `min_predecessors`. Add
`min_predecessors: None` to keep the previous behaviour — release the join when all
direct predecessors have arrived. `Some(n)` releases after *n* of them.

```rust
// 2.0
DeferredNodeConfig { timeout: Some(Duration::from_secs(30)), min_predecessors: None }
```

Two methods became fallible, so add `?` or `.unwrap()`:

| Method | 1.0 | 2.0 |
|--------|-----|-----|
| `CompiledGraph::get_next_nodes` | `Vec<String>` | `Result<Vec<String>>` — an undeclared route key is now an error rather than a silently stopped branch |
| `CompiledGraph::time_travel` | `TimeTravelHandle` (panicked with no checkpointer) | `Result<TimeTravelHandle>` |

Fan-in behaviour changed without an API change: a node with more than one incoming
direct edge is now deferred automatically, so it runs once after its branches
arrive rather than once per arrival. A graph that relied on the node running
repeatedly needs restructuring; a graph that worked around the old behaviour can
drop the workaround.
...
ADK-Rust now uses the official `rmcp 3.1` SDK. Code that names `rmcp` content,
elicitation, or service types directly must import them through the re-export so
...
The client still advertises MCP `2025-11-25`, so existing servers are unaffected.
A `2026-07-28` server answers the same handshake. The new stateless
`server/discover` handshake is opt-in through
`adk_tool::mcp::ClientLifecycleMode`.

Three changes need code edits:

| Was | Now |
|-----|-----|
| `ConnectionRefresher::call_tool` returned `RetryResult<CallToolResult>` | returns `RetryResult<CallToolResponse>` |
| `SimpleClient::call_tool` returned `CallToolResult` | returns `CallToolResponse` |
| A tool declared its own task contract | The server decides per call |

`CallToolResponse` distinguishes what the server did, so match on it:

```rust
use adk_tool::mcp::rmcp::model::CallToolResponse;

match response {
    CallToolResponse::Complete(result) => { /* the usual inline result */ }
    CallToolResponse::Task(created) => { /* poll tasks/get for the result */ }
    other => { /* SEP-2322 input_required, or a revision added later */ }
}
```

`McpToolset::execute` handles all three internally, so agents using
`McpToolset` need no change.

Because SEP-2663 removed the per-tool task contract, `Tool::is_long_running`
now answers per connection: true when tasks are enabled and the server
negotiated them. A `CodeActAgent` that suspends on long-running tools therefore
suspends for any tool on a task-capable server rather than only the tools that
declared it.

MCP sampling remains available as an opt-in deprecated-compatibility feature under
upstream SEP-2577. Under `mcp-sampling`, `rmcp::model::SamplingMessageContent` is
now `SamplingMessageContentBlock`.

## ACP: agent-client-protocol 2.2

`adk-acp` now uses the official `agent-client-protocol` 2.2 SDK while negotiating
stable wire protocol v1. Code that uses the re-exported `adk_acp::agent_client_protocol`
directly compiles against the SDK's 2.x API. Capability publication is exact: unsupported media,
