...
```toml
# Only Gemini
adk-model = { version = "0.9.2", default-features = false, features = ["gemini"] }

# Mixed providers (e.g., Gemini + Anthropic)
adk-model = { version = "0.9.2", default-features = false, features = ["gemini", "anthropic"] }

# Ollama only (no API key needed)
adk-model = { version = "0.9.2", default-features = false, features = ["ollama"] }
```
