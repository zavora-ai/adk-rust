...
async fn test_callback_execution() {
        ...
        Box::pin(async move {
            *flag.lock().unwrap() = true;
            Ok(Some(Content {
                ...
                parts: vec![Part::Text {
                    text: "Before callback executed".to_string(),
                }],
            }))
        }) as std::pin::Pin<Box<dyn std::future::Future<Output = adk_core::Result<Option<Content>>> + Send>>
    });
        ...
        Box::pin(async move {
            *flag.lock().unwrap() = true;
            Ok(Some(Content {
                role: "system".to_string(),
                parts: vec![Part::Text {
                    text: "After callback executed".to_string(),
                }],
            }))
        }) as std::pin::Pin<Box<dyn std::future::Future<Output = adk_core::Result<Option<Content>>> + Send>>
    });
    ...
    assert_eq!(before_result.unwrap().parts[0], Part::Text {
        text: "Before callback executed".to_string(),
    });
    ...
    assert_eq!(after_result.unwrap().parts[0], Part::Text {
        text: "After callback executed".to_string(),
    });
...
async fn test_callback_error_handling() {
    let error_callback = Box::new(|_ctx: Arc<dyn CallbackContext>| {
        Box::pin(async move {
            Err(adk_core::AdkError::Agent("Callback error".to_string()))
        }) as std::pin::Pin<Box<dyn std::future::Future<Output = adk_core::Result<Option<Content>>> + Send>>
    });
...
async fn test_callback_returns_none() {
    let none_callback = Box::new(|_ctx: Arc<dyn CallbackContext>| {
        Box::pin(async move { Ok(None) }) as std::pin::Pin<Box<dyn std::future::Future<Output = adk_core::Result<Option<Content>>> + Send>>
    });
