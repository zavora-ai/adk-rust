...
use adk_core::{
    ...
    Result, RunConfig, Session, State, Tool, ToolContext, FinishReason,
};
...
impl adk_core::ReadonlyContext for MockContext {
    fn invocation_id(&self) -> &str { "inv-real" }
    fn agent_name(&self) -> &str { "gemini-agent" }
    fn user_id(&self) -> &str { "user-real" }
    fn app_name(&self) -> &str { "test-app" }
    fn session_id(&self) -> &str { "session-real" }
