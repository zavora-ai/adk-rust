...
impl Session for MockSession {
    fn id(&self) -> &str { "stream-bench-session" }
    fn app_name(&self) -> &str { "stream-bench-app" }
    fn user_id(&self) -> &str { "stream-bench-user" }
...
impl ReadonlyContext for BenchContext {
    fn invocation_id(&self) -> &str { "stream-bench-inv" }
    fn agent_name(&self) -> &str { "stream-bench-agent" }
    fn user_id(&self) -> &str { "stream-bench-user" }
    fn app_name(&self) -> &str { "stream-bench-app" }
    fn session_id(&self) -> &str { "stream-bench-session" }
...
fn benchmark_time_to_first_token(c: &mut Criterion) {
    ...
    c.bench_function("streaming_time_to_first_token", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let agent = LlmAgentBuilder::new("stream-bench-agent")
                .model(model)
                .build()
                .unwrap();
...
fn benchmark_streaming_throughput(c: &mut Criterion) {
    ...
    c.bench_function("streaming_full_throughput", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let ctx = Arc::new(BenchContext::new(black_box("Explain quantum computing in 2 paragraphs.")));
            let mut stream = agent.run(ctx).await.unwrap();
