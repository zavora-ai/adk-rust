...
async fn test_basic_toolset() {
    let tools: Vec<Arc<dyn Tool>> = vec![
        Arc::new(ExitLoopTool::new()),
        Arc::new(GoogleSearchTool::new()),
    ];
...
async fn test_toolset_with_predicate() {
    ...
    let toolset = BasicToolset::new("filtered_toolset", tools)
...
async fn test_string_predicate_multiple() {
    ...
    let toolset = BasicToolset::new("all_tools", tools)
        .with_predicate(predicate);
...
async fn test_empty_predicate() {
    ...
    let predicate = string_predicate(vec![]);
    let toolset = BasicToolset::new("no_tools", tools)
        .with_predicate(predicate);
